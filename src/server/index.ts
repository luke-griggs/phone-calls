...
import { listHumanCalls, getHumanCallStats } from "../db/queries.js";
...
app.get("/", (c) => {
  return c.json({
    status: "ok",
    service: "vapi-human-conversations",
...
app.get("/api/calls", async (c) => {
  ...
  try {
    const calls = await listHumanCalls(limit);
...
app.get("/api/stats", async (c) => {
  try {
    const stats = await getHumanCallStats();
...
console.log(`
╔═══════════════════════════════════════════════════════════╗
║       Vapi Human Conversation Tracking Server             ║
╠═══════════════════════════════════════════════════════════╣
║  Endpoints:                                               ║
║  - GET  /              Health check                       ║
